<?php
  ...
  // Set graph title
  $graph->title->Set("Display faculty members by rank");
  ...
  if($_SESSION['dept1']=="All")
    $sql = pg_query($conn, "SELECT DISTINCT rank FROM faculty")or die("Query failed with error: ".pg_last_error($conn));
  else
    $sql = pg_query($conn, "SELECT DISTINCT rank FROM faculty WHERE dept_id='".$_SESSION['dept1']."'")or die("Query failed with error: ".pg_last_error($conn));
  while ($row = pg_fetch_array($sql))
  {
    $legends[] = $row['rank'];
    if($_SESSION['dept1']=="All")
      $sql2 = pg_query($conn, "SELECT COUNT(*) FROM faculty WHERE rank='".$row['rank']."'")or die("Query failed with error: ".pg_last_error($conn));
    else
      $sql2 = pg_query($conn, "SELECT COUNT(*) FROM faculty WHERE dept_id='".$_SESSION['dept1']."' AND rank='".$row['rank']."'")or die("Query failed with error: ".pg_last_error($conn));
    ...
    $values[] = $count[0];
    $targs[] = "#";
    $alts[] = $row['rank']." (".$count[0]."/".$count2[0].")";
  }
  ...
  if($_SESSION['dept2']=="All")
    ...
    $sql = pg_query($conn, "SELECT DISTINCT rank FROM faculty WHERE dept_id='".$_SESSION['dept2']."'")or die("Query failed with error: ".pg_last_error($conn));
  while ($row = pg_fetch_array($sql))
  {
    $legends2[] = $row['rank'];
    if($_SESSION['dept2']=="All")
      ...
      $sql2 = pg_query($conn, "SELECT COUNT(*) FROM faculty WHERE dept_id='".$_SESSION['dept2']."' AND rank='".$row['rank']."'")or die("Query failed with error: ".pg_last_error($conn));
    $count = pg_fetch_row($sql2);
    if($_SESSION['dept2']=="All")
      $sql2 = pg_query($conn, "SELECT COUNT(*) FROM faculty")or die("Query failed with error: ".pg_last_error($conn));
    else
    ...
    $values2[] = $count[0];
    $targs2[] = "#";
    $alts2[] = $row['rank']." (".$count[0]."/".$count2[0].")";
  }
