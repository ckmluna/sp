...
<body>
<?php
  if(isset($_GET['deleted'])) echo "<div class=\"notif\">College successfully deleted.</div>";
?>
<div id="masthead">
  ...
  <div id="content2" style="font-size:small;">
    <?php
      ...
      if(isset($_POST['sortorder']))
        $sql = pg_query($conn, "SELECT * FROM college ORDER BY name ".$_POST['sortorder'])or die("Query failed with error: ".pg_last_error($conn));
      else
        $sql = pg_query($conn, "SELECT * FROM college ORDER BY name ASC")or die("Query failed with error: ".pg_last_error($conn));
      echo "<table>";
      echo "<thead><th>Name</th><th></th><th></th></thead>";
      while ($row = pg_fetch_array($sql))
      {
        echo "<tr><td>".$row['name']."</td>";
        echo "<td>[<a href=\"viewcollegeinfo.php?id=".$row['id_no']."\">Edit</a>]</td>";
        echo "<td>[<a href=\"deletecollege.php?id=".$row['course_id']."\">Delete</a>]</tr></td>";
      }
      echo "</table>";
    ?>
  </div>
